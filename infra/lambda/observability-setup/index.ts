...
async function ensureDeliverySource(
  ...
  try {
    ...
    if (err instanceof Error && (err.name === 'ConflictException' || err.name === 'ResourceAlreadyExistsException')) {
      return;
    }
    throw err;
  }
...
async function onDelete(event: CfnEvent): Promise<CfnResponse> {
  ...
  try {
    ...
    console.warn('Cleanup error (non-fatal):', message);
  }
...
export async function handler(event: CfnEvent): Promise<CfnResponse> {
  ...
  switch (event.RequestType) {
    ...
    default: {
      const _exhaustive: never = event.RequestType;
      throw new Error(`Unknown RequestType: ${_exhaustive}`);
    }
  }
}
