...
Each line begins with a speaker label followed by their utterance. The label is either the participant's real name ("Tiago: ..."), an explicit anonymous id ("Speaker 2: ..."), or a legacy bracketed form ("[Speaker 2] ..."). Treat each distinct label as a distinct participant. Never invent, guess, or promote a name for an anonymous speaker — if the label is "Speaker 2" or "[Speaker 2]", refer to them exactly that way.
...
4. Attribute clearly. Prefer "<label> explained/asked/committed to ..." over faceless third-party prose, using each speaker's actual label — a real name where given, "Speaker N" / "[Speaker N]" where anonymous. Quote a pivotal line verbatim when its exact wording carries meaning, using the form <label>: "the quoted line". Do not neutralize the speakers' voices into anonymous summary, and do not assign a name to an anonymous speaker.
