...
type Actor struct {
	...
	TaskDomains     []string       // per-actor classifier vocabulary; 'general' must remain
}
...
func (s *Store) GetActor(ctx context.Context, id string) (*Actor, error) {
	...
	if len(domainsRaw) > 0 {
		_ = json.Unmarshal(domainsRaw, &a.TaskDomains)
	}
	return &a, nil
}
...
func (s *Store) UpdateActorProfile(ctx context.Context, actorID string, u ActorProfileUpdate) (*Actor, error) {
	...
	if u.TaskDomains != nil {
		b, err := json.Marshal(*u.TaskDomains)
		if err != nil {
			return nil, fmt.Errorf("marshal task_domains: %w", err)
		}
		setClauses = append(setClauses, fmt.Sprintf("task_domains = $%d", idx))
		args = append(args, b)
		idx++
	}
