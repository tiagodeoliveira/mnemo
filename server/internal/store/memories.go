...
// ListNamespaceForConsolidation returns up to `limit` items from a namespace,
// ordered by (reinforced_count DESC, updated_at DESC). Bounds the LLM prompt
// size for extract_context consolidation, where hot namespaces would
// otherwise push the prompt past the model's context window.
func (s *Store) ListNamespaceForConsolidation(ctx context.Context, actorID, namespace string, limit int) ([]Memory, error) {
	rows, err := s.DB.QueryContext(ctx, `
		SELECT memory_id, actor_id, dimension, namespace, content,
		       tags, attributes, source_event_ids, reinforced_count,
		       created_at, updated_at, expires_at
		  FROM memories
		 WHERE actor_id = $1 AND namespace = $2
		 ORDER BY reinforced_count DESC, updated_at DESC
		 LIMIT $3
	`, actorID, namespace, limit)
	if err != nil {
		return nil, err
	}
	defer func() { _ = rows.Close() }()
...
func (s *Store) ListItems(ctx context.Context, opts ListItemsOpts) ([]Memory, error) {
	...
	// Exclude expired rows so decayed one-off facts drop out of recall
	// immediately, before the background sweep deletes them. NULL = permanent.
...
// DeleteExpiredMemories hard-deletes rows whose expires_at has passed. Recall
// already filters these out lazily; this reclaims the space and, crucially,
// keeps namespaces small enough that consolidation's per-call item cap doesn't
// blind it to existing facts. Returns the number of rows deleted.
