...
// TestApplyMemoryDiffInsertDedupes verifies the semantic-dedup backstop:
// when an INSERT op carries an embedding nearly identical to an existing
// row's embedding (cosine >= 0.85), the insert is collapsed into a
// REINFORCE of the existing row.
func TestApplyMemoryDiffInsertDedupes(t *testing.T) {
	dsn := startPG(t)
	s, _ := Open(context.Background(), dsn)
	defer func() { _ = s.Close() }()
	if err := s.Migrate(); err != nil {
	...
	ns := "/preferences/alice/"
	originalEvent := uuid.New()
	// Seed an existing row with a known embedding.
	existingVec := makeVec(1.0)
	existingID := uuid.New()
	if _, err := s.DB.ExecContext(ctx, `
		...
		) VALUES ($1, 'alice', 'preferences', $2, 'prefers Go for backend',
		          '[]', '{}', ARRAY[$3::uuid], 1, now(), now(), $4)
	`, existingID, ns, originalEvent, pgvector.NewVector(existingVec)); err != nil {
		t.Fatal(err)
	}

	// Apply a diff whose INSERT carries a near-identical embedding (cosine ~1.0).
	newEvent := uuid.New()
	opts := DiffApplyOpts{
		ActorID:       "alice",
		Dimension:     "preferences",
		Namespace:     ns,
		SourceEventID: newEvent,
		TTLDays:       365,
	}
	diff := MemoryDiff{
		Insert: []InsertOp{
			{
				...
				Embedding: makeVec(1.0), // identical → cosine = 1.0, well above 0.85
			},
		},
	}
	...
	if len(items) != 1 {
		t.Fatalf("expected dedup to collapse insert; want 1 row, got %d", len(items))
	}
	if items[0].ID != existingID {
		t.Fatalf("expected the existing row to survive; got id %s", items[0].ID)
	}
	if items[0].ReinforcedCount != 2 {
		t.Fatalf("expected reinforced_count=2 after dedup, got %d", items[0].ReinforcedCount)
	}
	if items[0].Content != "prefers Go for backend" {
		t.Fatalf("dedup must not rewrite content; got %q", items[0].Content)
	}
}

// TestApplyMemoryDiffInsertBelowDedupThreshold verifies that a candidate with
// cosine < threshold passes through as a regular insert, producing two rows.
func TestApplyMemoryDiffInsertBelowDedupThreshold(t *testing.T) {
	dsn := startPG(t)
	s, _ := Open(context.Background(), dsn)
	defer func() { _ = s.Close() }()
	if err := s.Migrate(); err != nil {
	...
	dim := 1536
	makeVec := func(values ...float32) []float32 {
		v := make([]float32, dim)
		for i := range v {
			v[i] = values[i%len(values)]
		}
		var norm float32
		for _, x := range v {
			norm += x * x
	...
	ns := "/preferences/alice/"
	originalEvent := uuid.New()
	// Existing vector: alternating 1, 0 components.
	if _, err := s.DB.ExecContext(ctx, `
		INSERT INTO memories (
			memory_id, actor_id, dimension, namespace, content,
			tags, attributes, source_event_ids, reinforced_count,
			created_at, updated_at, embedding
		) VALUES (gen_random_uuid(), 'alice', 'preferences', $1, 'first',
		          '[]', '{}', ARRAY[$2::uuid], 1, now(), now(), $3)
	`, ns, originalEvent, pgvector.NewVector(makeVec(1, 0))); err != nil {
		t.Fatal(err)
	}

	// New insert: orthogonal vector — cosine = 0, well below the 0.85 threshold.
	diff := MemoryDiff{
		Insert: []InsertOp{
			{
				Content:   "second",
	...
	tx, _ := s.DB.BeginTx(ctx, nil)
	if err := s.ApplyMemoryDiff(ctx, tx, DiffApplyOpts{
		ActorID: "alice", Dimension: "preferences", Namespace: ns,
		SourceEventID: uuid.New(), TTLDays: 365,
	}, diff); err != nil {
		t.Fatal(err)
	}
	_ = tx.Commit()

	items, _ := s.ListItemsByNamespace(ctx, "alice", ns)
	if len(items) != 2 {
