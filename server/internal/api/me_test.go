...
func TestMeGetReturnsSeededTaskDomains(t *testing.T) {
	srv := setupMeTestServer(t)
	...
	if len(result.TaskDomains) != len(want) {
		t.Fatalf("task_domains: got %v, want %v", result.TaskDomains, want)
	}
	for i, d := range want {
		if result.TaskDomains[i] != d {
			t.Fatalf("task_domains[%d]: got %q, want %q", i, result.TaskDomains[i], d)
		}
	}
}

func TestMePatchTaskDomainsHappyPath(t *testing.T) {
	srv := setupMeTestServer(t)

	body, _ := json.Marshal(map[string]any{
		"task_domains": []string{"coding", "data-analysis", "writing", "general"},
	})
	req, _ := http.NewRequest(http.MethodPatch, srv.URL+"/me", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	...
	if len(result.TaskDomains) != 4 || result.TaskDomains[1] != "data-analysis" {
		t.Fatalf("task_domains: got %v", result.TaskDomains)
	}
}

func TestMePatchTaskDomainsRejectsMissingGeneral(t *testing.T) {
	srv := setupMeTestServer(t)

	body, _ := json.Marshal(map[string]any{
		"task_domains": []string{"coding", "studying"},
	})
	req, _ := http.NewRequest(http.MethodPatch, srv.URL+"/me", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
...
func TestMePatchTaskDomainsRejectsBadSlug(t *testing.T) {
	srv := setupMeTestServer(t)
	...
	cases := []struct {
		...
		{"underscore", []string{"data_analysis", "general"}},
		{"space", []string{"data analysis", "general"}},
		{"empty entry", []string{"", "general"}},
		{"leading dash", []string{"-coding", "general"}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			body, _ := json.Marshal(map[string]any{"task_domains": tc.in})
			req, _ := http.NewRequest(http.MethodPatch, srv.URL+"/me", bytes.NewReader(body))
			req.Header.Set("Content-Type", "application/json")
			resp, err := http.DefaultClient.Do(req)
			if err != nil {
...
func TestMePatchTaskDomainsAutoLowercases(t *testing.T) {
	// Mixed-case input is friendly: lowercase + trim, then validate the slug shape.
	// The CLI also lowercases on its side, but the server is the authority.
	srv := setupMeTestServer(t)

	body, _ := json.Marshal(map[string]any{
		"task_domains": []string{"Coding", "  Writing  ", "General"},
	})
	req, _ := http.NewRequest(http.MethodPatch, srv.URL+"/me", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	...
	want := []string{"coding", "writing", "general"}
	for i, d := range want {
		if result.TaskDomains[i] != d {
			t.Fatalf("task_domains[%d] = %q, want %q", i, result.TaskDomains[i], d)
		}
	}
}

func TestMePatchTaskDomainsDedupes(t *testing.T) {
	srv := setupMeTestServer(t)

	body, _ := json.Marshal(map[string]any{
		"task_domains": []string{"coding", "coding", "general", "general"},
	})
	req, _ := http.NewRequest(http.MethodPatch, srv.URL+"/me", bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	resp, err := http.DefaultClient.Do(req)
	...
	if len(result.TaskDomains) != 2 {
		t.Fatalf("expected deduped to 2 entries, got %v", result.TaskDomains)
	}
}
