...
//
// metrics is optional; pass nil to disable emission. Each breaker is named
// "llm.<provider>" (e.g. "llm.anthropic") so a CircuitBreakerOpen alert
// fires with a meaningful `name` label.
func NewChain(providers []Provider, threshold int, cooldown time.Duration, logger *slog.Logger, metrics BreakerMetrics) *Chain {
