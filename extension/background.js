...
async function saveCredentials(t) {
  if (typeof t.expires_in !== 'number' || t.expires_in <= 0) {
    throw new Error(`auth0 token response missing positive expires_in: ${t.expires_in}`);
  }
  const cred = {
...
async function _doRefresh(cfg) {
  ...
  if (typeof t.expires_in !== 'number' || t.expires_in <= 0) return null;
  const fresh = {
    access_token: t.access_token,
...
// Keep TOKEN_REFRESH_SKEW_SECONDS in sync with cli/src/auth.ts — both guard
// the same access-token refresh boundary against client/server clock drift.
const TOKEN_REFRESH_SKEW_SECONDS = 60;

async function getAccessToken(cfg) {
  const cur = await loadCredentials();
  if (!cur) return null;
  if (cur.expires_at - TOKEN_REFRESH_SKEW_SECONDS > Math.floor(Date.now() / 1000)) {
