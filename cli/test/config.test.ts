...
describe('config loader', () => {
  ...
  it('falls back to DEFAULTS auth0 values when fields are absent', () => {
    const config = { apiUrl: 'https://api.example.com/v1', defaults: { visible: true } };
    fs.writeFileSync(configPath, JSON.stringify(config));

    const loaded = loadConfig(configPath);
    // Exact equality (not toBeTruthy) — committed defaults are empty strings
    // until release CI stamps real values from repo Variables. The test
    // verifies the fallback path itself; the values it falls back *to* are
    // whatever this build was compiled with.
    expect(loaded.auth0Domain).toBe(DEFAULTS.auth0Domain);
    expect(loaded.auth0Audience).toBe(DEFAULTS.auth0Audience);
    expect(loaded.auth0ClientId).toBe(DEFAULTS.auth0ClientId);
  });
