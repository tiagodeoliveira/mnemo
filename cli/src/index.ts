...
import {
  configDomainsListCmd,
  configDomainsAddCmd,
  configDomainsRemoveCmd,
  configDomainsResetCmd,
} from './commands/config-domains';
import { bootstrapCmd } from './commands/bootstrap';
...
const domainsCmd = configCmd
  .command('domains')
  .description("View or update the classifier's task-domain list ('general' always present)")
  .action(async () => {
    try {
      await configDomainsListCmd();
    } catch (err: unknown) {
      process.stderr.write(`mnemo config domains error: ${err instanceof Error ? err.message : String(err)}\n`);
      process.exit(1);
    }
  });
...
domainsCmd
  .command('add <names...>')
  .description('Add one or more task domains')
  .action(async (names: string[]) => {
    try {
      await configDomainsAddCmd(names);
    } catch (err: unknown) {
      process.stderr.write(`mnemo config domains add error: ${err instanceof Error ? err.message : String(err)}\n`);
      process.exit(1);
    }
  });
...
domainsCmd
  ...
  .description("Remove one or more task domains (cannot remove 'general')")
  .action(async (names: string[]) => {
    try {
      await configDomainsRemoveCmd(names);
    } catch (err: unknown) {
      process.stderr.write(`mnemo config domains remove error: ${err instanceof Error ? err.message : String(err)}\n`);
      process.exit(1);
    }
  });
...
domainsCmd
  .command('reset')
  .description('Reset task domains to defaults')
  .action(async () => {
    try {
      await configDomainsResetCmd();
    } catch (err: unknown) {
      process.stderr.write(`mnemo config domains reset error: ${err instanceof Error ? err.message : String(err)}\n`);
      process.exit(1);
    }
  });
...
program
  .command('bootstrap')
  .description('Ingest a document (README, docs, source file) into memory as one or more synthetic events')
  .requiredOption('--file <path>', 'Path to the document to ingest')
  .option('--source <label>', 'Override the source label (defaults to the filename)')
  .option('--project <name>', 'Route extracted project facts into this project namespace')
  .action(async (opts) => {
    try {
      await bootstrapCmd(opts);
    } catch (err: unknown) {
      process.stderr.write(`mnemo bootstrap error: ${err instanceof Error ? err.message : String(err)}\n`);
      process.exit(1);
    }
  });
