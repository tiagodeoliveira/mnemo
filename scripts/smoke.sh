...
#
# Env overrides:
#   MNEMO_SMOKE_PORT      — port the server binds (default 18080)
#   MNEMO_SMOKE_PG_PORT   — host-side Postgres port (default 55432)
...
#   MNEMO_SMOKE_KEEP_DATA=1 — skip the pre-run volume wipe (use only when
#                             debugging an already-populated DB).
#   MNEMO_ANTHROPIC_MODEL — override the Claude model (default claude-sonnet-4-6)
...
PORT="${MNEMO_SMOKE_PORT:-18080}"
PG_PORT="${MNEMO_SMOKE_PG_PORT:-55432}"
export MNEMO_DEV_PG_PORT="$PG_PORT"
TIMEOUT="${MNEMO_SMOKE_TIMEOUT:-180}"
KEEP="${MNEMO_SMOKE_KEEP:-0}"
MODEL="${MNEMO_ANTHROPIC_MODEL:-claude-sonnet-4-6}"
...
ACTOR="dev-actor"
DATE="$(date -u +%Y-%m-%d)"
DSN="postgres://mnemo:mnemo@localhost:${PG_PORT}/mnemo?sslmode=disable"
...
bring_up_postgres() {
  ...
  # Wipe the volume by default so each smoke run starts from an empty DB.
  # Dimension assertions are tightly ranged (e.g. preferences in [2..12]); a
  # persistent volume would carry rows from prior runs and false-fail them.
  # Set MNEMO_SMOKE_KEEP_DATA=1 to skip the wipe when debugging.
  if [[ "${MNEMO_SMOKE_KEEP_DATA:-0}" != "1" ]]; then
    info "wiping mnemo-pg-data volume for a clean run (MNEMO_SMOKE_KEEP_DATA=1 to skip)"
